...
for i in range(1, 12):
    temp = 'temp_{}'.format(i)
    load = 'load_{}'.format(i)
    plt.scatter(temps[temp], loads[load], c='xkcd:{}'.format(colors[i-1]),alpha = 0.01)
    plt.title('Temperature versus Load for zone {}'.format(i))
    plt.xlabel('Temperature (fahrenheit)')
    plt.ylabel('Load (kWh)?')
    plt.savefig('plots/tempvsload{}_transparent'.format(i), bbox_inches='tight')
    plt.clf()


#Plot a day in winter for each zone - Jan 17
for i in range(1, 22):
    avg = np.average(loads['load_{}'.format(i)][384:408])
    
    plt.plot(loads['time_{}'.format(i)][384:408], loads['load_{}'.format(i)][384:408]/avg, label = 'Zone.' + str(i))
    plt.legend()
    plt.savefig('plots/Winter_jan_17_normalized', bbox_inches='tight') 
    

#Plot a day in winter for each zone - Jul 26
for i in range(1, 22):
    avg = np.average(loads['load_{}'.format(i)][4968:4991])
    
    plt.plot(loads['time_{}'.format(i)][4968:4991], loads['load_{}'.format(i)][4968:4991]/avg, label = 'Zone.' + str(i))
    plt.legend()
    plt.savefig('plots/Summer_jul_26_04_normalized', bbox_inches='tight') 
